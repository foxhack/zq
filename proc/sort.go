...
func newRunFile(filename string, sorted []*zng.Record, zctx *resolver.Context) (*runFile, error) {
	...
	r := &runFile{file: f}
	if err := writeBzng(f, sorted); err != nil {
		r.closeAndRemove()
		return nil, err
	}
	...
	zr := bzngio.NewReader(bufio.NewReader(f), zctx)
	rec, err := zr.Read()
	if err != nil {
		r.closeAndRemove()
		return nil, err
...
// writeBzng writes records to w as a bzng stream.
func writeBzng(w io.Writer, records []*zng.Record) error {
	bw := bufio.NewWriter(w)
	zw := bzngio.NewWriter(bw)
